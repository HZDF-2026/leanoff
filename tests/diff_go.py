"""Differential test: the Go port must behave exactly like the Python
...
module, which also exercises the fail-fast build path.

Usage:  python tests/diff_go.py
...
GO_BIN = ROOT / "dist" / "go" / ("leanoff.exe" if os.name == "nt" else "leanoff")
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        for fmt in ("json", "text"):
            for cmd in ("verify", "build"):
                ...
                go = subprocess.run([str(GO_BIN)] + args,
                                    capture_output=True, text=True)
                label = f"{cmd} {fmt}"
                if py.returncode != go.returncode:
                    print(f"FAIL {label}: exit {py.returncode} vs {go.returncode}")
                    print("py stderr:", py.stderr)
                    print("go stderr:", go.stderr)
                    failures += 1
                    continue
                if fmt == "json":
                    match = strip_timings_json(py.stdout) == strip_timings_json(go.stdout)
                else:
                    match = strip_timings_text(py.stdout) == strip_timings_text(go.stdout)
                print(f"{'ok  ' if match else 'FAIL'} {label}")
                if not match:
                    print("py stdout:", py.stdout)
                    print("go stdout:", go.stdout)
                    failures += 1
        if failures:
